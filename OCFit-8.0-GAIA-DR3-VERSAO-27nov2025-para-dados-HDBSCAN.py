...
ESTOU ATUALIZAND ESSE CODIGO = WILTON

esse programa ajusta isocronas aos dados gaia eDR3
le uma lista e busca todos os arquivos .csv gerados pelo HDBSCAN (ja formatados) de um diretorio
ajusta a iso em cada um deles
escreve os resultados num log e tambem no diretorio de cada arquivo


atencao:
    na rotina gaia_edr3_tools 
    no fit_iso_GAIA
    agora entro com a matriz com os dados ao inves de ler os dados novamente
    mudar isso nas rotinas das estacoes
...
# carregar tabelas (atencao: mudar o mypath no arquivo zpt.py)
#zpt.load_tables()
...
def transforma_para_float(array):
    ...
    saida = b = np.asfarray(saida,dtype = float)
    return saida
##########################################################################################
plt.close('all')
############################################################################
# setar diretorios
base_dir = '/home/wilton/ISO-FIT/'
#fit_dir = base_dir+'OCFit/'
memb_dir = '/home/wilton/ISO-FIT/dados-teste-HDBSCAN/'
dirout = memb_dir+'results/' 
...
for i in range(len(files)):
    ...
    if '__pycache__' in dirout:
           shutil.rmtree(dirout+'__pycache__')
           shutil.rmtree(dirout+'joblib')    
    ...
    obs = np.genfromtxt(files[i],names=True, delimiter=';', dtype=None)#, encoding='utf-8')
    ...
    ###########################################################################
    # para correcao de zero-point in Plx seguindo Leidegren 2021
    #zero_point = zpt.get_zpt(obs['phot_g_mean_mag'], obs['nu_eff_used_in_astrometry'], obs['pseudocolour'], obs['ecl_lat'], obs['astrometric_params_solved'])
    #Plx = Plx - zero_point
    #obs['parallax'] = obs['parallax'] - zero_point
    #data = data[np.isfinite(data['parallax'])]
    ###########################################################################
    ...
    #obs = add_col(obs,members,'probs')
    # obs = add_col(obs,Plx,'Plx')
    # obs = add_col(obs,ePlx,'e_Plx')
    # obs = add_col(obs,RA,'RA_ICRS')
    # obs = add_col(obs,DEC,'DE_ICRS')
    ###########################################################################
    
    ###########################################################################
    # calcula raio
    # para calcular o raio--------------------------------
    dist_center = pyasl.getAngDist(np.mean(RA),np.mean(DEC),RA,DEC)
    dist_center.sort()
    radius = max(dist_center) # in deg
    ...
    if (np.ravel(npoint).size > 5 and Gmag[ind_m].size < 10000):
        ...
        res_isoc, res_isoc_er = fit_iso_GAIA(obs,verbosefile,
        ...
        logfile.write('%10.4f;'%RA.mean())
        logfile.write('%10.4f;'%DEC.mean())

        logfile.write('%10.2f;'%radius)              
        ...
        # imprimir priors prior_dis;  e_prior_dist;    prior_age;  e_prior_age;  prior_Av;   e_prior_Av;  prior_FeH;  e_ prior_FeH;                             
        ...
        # falta imprimir median_likelihood;    16th_likelihood;  84th_likelihood
        ...
        plt.title(name)
        # plt.annotate(name,
        #             xy=(0.90, 0.95), xycoords='axes fraction',fontsize=12,
        #             xytext=(0.90, 0.95), textcoords='axes fraction',
