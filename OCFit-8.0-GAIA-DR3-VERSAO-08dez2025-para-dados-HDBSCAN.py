...
############################################################################
# setar diretorios
base_dir = '/home/wilton/CE-ISO-FIT/'
#fit_dir = base_dir+'OCFit/'
memb_dir = '/home/wilton/CE-ISO-FIT/dados-teste/'
...
for i in range(len(files)):
   ...
   # fit isochrone to the data
    ...
    if (np.ravel(npoint).size > 5 and Gmag[ind_m].size < 10000):
        ...
        R50pc_OC = math.ceil((radius50_OC*np.pi/180.)*res_isoc[1]*1.e3)
        ##########################################################################################
        # Raio em pc usando rmax
        Rmaxpc_OC = math.ceil((radiimax_OC*np.pi/180.)*res_isoc[1]*1.e3)
        ##########################################################################################
