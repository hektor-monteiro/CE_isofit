...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 24 05:27:17 2021
...
def PM_cluster_model(pars,data):
    ...
    phi_c = t1 * np.exp(-t2 * ( t3**2 + t4**2 - t5 ) )
...
def PM_field_model(pars,data):
    ...
    phi_f = t1 * np.exp(-t2 * ( t3**2 + t4**2 - t5 ) )
...
#    pmRA,pmDE,erRA,erDE = data
    ...
    phi_c = Nc * phi_c * weight
    phi_f = (1.-Nc) * phi_f  * (1.-weight)
...
def fit_plx_dr3(obs,weight,meanPLX,sigPLX):  
    ...
    prange = np.array([[0.01,0.99],
                       [meanPLX-1*sigPLX,meanPLX+1*sigPLX],
                       [0.01,sigPLX],
                       [obs.min(),obs.max()],
                       [0.1*obs.std(),2*obs.std()]])
...
###########################################################################################
        ...
        for n in range(sample):
            ind_low = np.where(pars[:,n] < prange[:,0] )
            pars[ind_low,n] = prange[ind_low,0]
            ind_hi = np.where(pars[:,n] > prange[:,1])
...
#    verbosefile.write('Starting isochrone fitting...\n')
    ...
    #obs = np.genfromtxt(obs_file,names=True, delimiter=';', dtype=None)
    #obs = np.genfromtxt(obs_file,names=True, dtype=None)
    obs = obs_file
    ...
    weight = obs['probs']# * (obs_oc['G_BPmag'].min()/obs_oc['G_BPmag'])**1
...
###########################################################################################
        ...
        if (iter > 10):
            ...
            avg_var = np.std(center[:,iter-10:iter],axis=1)/center[:,iter]
            #avg_var = np.max([avg_var[0], avg_var[1], avg_var[3], np.std(center[:,iter-10:iter],axis=1)[2]/0.1])
            avg_var = np.max([avg_var[0], avg_var[1], avg_var[3]])
            
        # if(covmat[2,2] < 0.001):
        #     covmat[2,2] = 0.1
            
#        print(pars[2,:])
        # print(iter, avg_var, 'center:',center[:,iter])
